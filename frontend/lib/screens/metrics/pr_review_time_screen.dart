    ...
    return MetricDetailScreen(
      ...
      onRefresh: () {
        ...
        for (final dev in devs) {
          ref.invalidate(prReviewTimeByDeveloperProvider(dev.login));
        }
      },
...
class _MultiDeveloperComparisonSection extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    for (var i = 0; i < developers.length; i++) {
      ...
      final dataAsync = ref.watch(prReviewTimeByDeveloperProvider(dev.login));
      ...
      dataAsync.when(
        ...
        data: (data) {
          if (data.count > 0) {
            dataList.add(RepoComparisonData(
              repoId: i + 1, // Use index as ID for developers
              repoName: dev.login,
              value: data.averageHours,
              color: _entityColors[i % _entityColors.length],
              formattedValue: formatDuration(data.averageHours),
    ...
    return RepoComparisonChart(
      data: dataList,
      title: 'Average PR Review Time by Developer',
      valueLabel: 'Hours',
      valueFormatter: (value) => formatDuration(value),
    );
  }
