...
class DeveloperStatsService:
    ...
    async def get_all_developers(
        ...
        for login, avatar in developers_map.items():
            stats = await self.get_developer_stats(
                login, start_date, end_date, repo_id
            )
            developers.append({
                "login": login,
                "avatar": avatar,
    ...
    async def get_developer_stats(
        ...
        avg_lines = (
            (total_additions + total_deletions) / prs_created
            if prs_created > 0
            else 0
        )
        ...
        # Commits made
        commits_q = select(func.count(Commit.id)).where(
